...
2D Stenosis Geometry-Conditioned PINN
    ...
    Functions to define BCs, data, PDE loss, building, 
        training, fine-tuning, and predicting.
...
# TODO: QUITE POSSIBLY BROKEN CURRENTLY! NEED MORE DEBUGGING!
class AnchorRegularizationCallback(dde.callbacks.Callback):
    ...
    Adds gradient contribution corresponding to L_anchor = (lambda/2)||theta-theta0||^2
    during backward via PyTorch parameter hooks so it affects optimizer updates.

    Also tracks simple diagnostics (norms) and prints per-batch and overall summaries.
    """

    def __init__(self, pretrained_weights, lambda_anchor: float = 0.01,
                 frozen_prefixes: list = None, print_every: float = 100):
        super().__init__()
        ...
        self.theta_0 = {name: tensor.detach().clone() for name, tensor in pretrained_weights}
        self._device = None
        self._hook_handles = []

        # diagnostics
        self.batch_anchor_norm = 0.0
        self.batch_grad_norm = 0.0
        self.batch_param_count = 0

        self.running_anchor_norm = 0.0
    ...
    def init(self):
        # register hooks once model is set
    ...
    def _register_hooks(self):
        ...
        for name, param in self.model.net.named_parameters():
            ...
            def make_hook(param, theta_0):
                def hook(grad):
                    # diagnostics before modifying grad
                    try:
                        anchor = self.lambda_anchor * (param.data - theta_0)
                        anchor_norm = float(anchor.norm().cpu().item())
                    except Exception:
                        anchor_norm = 0.0
                    try:
                        grad_norm = float(grad.norm().cpu().item()) if grad is not None else 0.0
                    except Exception:
                        grad_norm = 0.0

                    self.batch_anchor_norm += anchor_norm
                    self.batch_grad_norm += grad_norm
                    self.batch_param_count += 1

                    if grad is None:
                        return None
                    return grad.add_(anchor)

                return hook

            handle = param.register_hook(make_hook(param, theta_0))
    ...
    def on_batch_end(self):
        if self.batch_param_count == 0:
            return
        avg_anchor = self.batch_anchor_norm / float(self.batch_param_count)
        avg_grad = self.batch_grad_norm / float(self.batch_param_count)
        ratio = avg_anchor / (avg_grad + 1e-12)
        it = getattr(self.model.train_state, "iteration", None)
        if (it == 0) or (it % self.print_every == 0):
            print(f"[Anchor @ iter {it}] avg_anchor_norm={avg_anchor:.3e}, avg_grad_norm={avg_grad:.3e}, ratio={ratio:.3f}")

        self.running_anchor_norm += avg_anchor
        self.running_grad_norm += avg_grad
        self.running_steps += 1

    def on_train_end(self):
        # Remove hooks to avoid memory leaks when callback persists.
        ...
        if self.running_steps > 0:
            overall_anchor = self.running_anchor_norm / float(self.running_steps)
            overall_grad = self.running_grad_norm / float(self.running_steps)
            overall_ratio = overall_anchor / (overall_grad + 1e-12)
            print(f"[Anchor SUMMARY] avg_anchor_norm={overall_anchor:.3e}, avg_grad_norm={overall_grad:.3e}, ratio={overall_ratio:.3f}")
